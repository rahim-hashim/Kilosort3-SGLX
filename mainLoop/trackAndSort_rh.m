function [rez, st3, fWpc] = trackAndSort_rh(rez, figName, figPath, varargin)
  %% Adapted from trackAndSort.m
  %%
  %%  Includes updated make_fig_rh
  %%  which saves figures to figPath
  %%
  %%   Author:  Rahim Hashim
  %%   Created: Jan 2024
...
%     [UtU, maskU] = getMeUtU(iW, iC, mask, Nnearest, Nchan); % this needs to change (but I don't know why!)%     
    ...
    elseif ibatch==niter && ops.fig
        fprintf('Saving figures to %s\n', figPath)
        figHand = figure;
        save_fig(W, U, mu, nsp, figName, figPath)
    end
end
fclose(fid);
toc
